...
let tokens;
let choiceBet = 1;
let account = document.getElementById("compte").innerText;
console.log(account)
let tempAccount = document.getElementById("compte").innerText;
let even = 0;
let odd = 0;
let red = 0;
let black = 0;
...
const check = () => {
	...
	if(account>=tempAccount-currentlyBet){
        let platoide = account - (tempAccount - currentlyBet)
        console.log(account)
        console.log(platoide)
        if (platoide>0){
         ajaxPost("/sumardinero",{plata:account},function(data){
          });
          finish.innerText = "GANASTE " + (platoide) + " $";
        }
        else{
          console.log(currentlyBet)
          ajaxPost("/menordinero",{plata:account},function(data){
          });
           finish.innerText = "PERDISTE " + (currentlyBet) + " $";
        }
    }
	else {
        console.log(valBet)
        ...
        finish.innerText = "PERDISTE " + (valbet) + " $";
    }
	if(currentlyBet<=0)finish.innerText = "Pon tu apuesta";
};
